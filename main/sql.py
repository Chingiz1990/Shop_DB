...
# Poduct.objects.only('id', 'name')

# SELECT description, price, category_id FROM product;
# Product.objeccts.defer('id', 'name')
...
# Product.objects.filter(price__gte=50000).filter(lte=60000)
...
# SELECT * FROM product WHERE LIKE 'Samsung';
# Product.objects.filter(name__exact='Samsung)

# SELECT * FROM product WHERE ILIKE 'Samsung';
